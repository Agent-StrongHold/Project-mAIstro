...
- **Tool-result governance is pinned across real Agent strategies (#1202,
  ...
  Sentinel. It checks that a PII-bearing or prompt-injection tool result
  reaches the model as the same redacted text or Sentinel refusal, and never
  raw, for every strategy. Test-only; no runtime behavior changes.
