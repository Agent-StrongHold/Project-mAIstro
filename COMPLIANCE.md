...
CI validates the registry on every change. The release workflow runs the same check with `--require-release-evidence --release-digest <tag commit>`. That mode fails closed for every `implemented` claim marked `release_required`; non-green statuses remain explicit rather than being silently promoted to green. This makes a release possible while the registry honestly records controls that are still partial, documented, planned, not applicable, or unverified.

An evidence record must identify its control ID and exactly repeat that control's executable control and test references. The evidence link must identify an immutable GitHub Actions artifact, carry its SHA-256, record the observed date and result, and name the exact release commit digest. Green `implemented` evidence must use an artifact URL and include an attestation naming the same control and every test reference. The checker downloads that artifact and verifies the attestation's SHA-256, control/test binding, and passing exit codes; generic successful run metadata is not evidence that the claimed tests executed. It also queries GitHub for the artifact digest, workflow run conclusion, workflow path, release head SHA, artifact expiry, and live workflow state, failing closed when any provenance is missing or mismatched. Disabled, manual-only, never-run, stale, or failing evidence cannot support green status. The `compliance-evidence.yml` workflow runs `scripts/produce-compliance-evidence.py` on protected branch and release pushes; that producer emits the attestations which a reviewed registry evidence record can link immutably. Every `test_refs` entry must be an existing Python test module under `tests/`, a package `tests/` directory, or `formal/`, with a `test_` filename; a path alone is not evidence that the test ran.
