...
An evidence record must identify its control ID and exactly repeat that control's executable control and test references. Its URL is either an immutable Actions artifact URL or a commit-bound workflow locator (`blob/<release_digest>/.github/workflows/...`). The latter is used for first-party evidence because an artifact ID is assigned only after the workflow runs; the checker resolves the locator to the exact successful run and deterministic `compliance-evidence-<release_digest>` artifact, so adding an artifact URL cannot change the release digest. Direct artifact records carry their SHA-256; locator records use `sha256: null` and the checker obtains and verifies the artifact's GitHub SHA-256. Green `implemented` evidence must include an attestation naming the same control and every test reference. The checker downloads that artifact and verifies the attestation's optional manifest SHA-256, control/test binding, and passing exit codes; generic successful run metadata is not evidence that the claimed tests executed. It also queries GitHub for the artifact digest, workflow run conclusion, workflow path, release head SHA, artifact expiry, and live workflow state, failing closed when any provenance is missing or mismatched. Disabled, manual-only, never-run, stale, or failing evidence cannot support green status. The `compliance-evidence.yml` workflow runs `scripts/produce-compliance-evidence.py`, uploads the immutable artifact, and verifies it against the same commit; it deliberately does not run on release tags because branch evidence for the exact tag commit must exist before publication. Every `test_refs` entry must be an existing Python test module under `tests/`, a package `tests/` directory, or `formal/`, with a `test_` filename; a path alone is not evidence that the test ran.
