...
| Browser/Chromium navigation SSRF protection | **Present at separate Playwright seams** — `maistro.tools.browser.guard::BrowserNetworkGuard` (async) and `SyncBrowserNetworkGuard` (sync) route every request and redirect hop through the same canonical policy. The browser-use provider (`maistro.tools.browser.client::BrowserClient`, which also covers the Conductor agent tools' web-search/browse fallback in `packages/hive-conductor/backend/services/tool_executor.py` — the only path by which agent tools reach Chromium), Conductor UI hill-climber (`packages/hive-conductor/backend/services/ui_auto_climb.py::screenshot`), dashboard screenshot route (`packages/hive-conductor/backend/routes/widgets.py::capture_screenshot`), and standalone hill-climb drivers (`packages/hive-conductor/run_hill_climb.py` and `packages/hive-conductor/hill-climb-ui.sh`, the script the Hyperlight UI-climb workload dispatches) attach the appropriate guard before navigation; service workers are blocked. Operator allowances are exact origins from `BROWSER_USE_ALLOWED_ORIGINS`, with the dashboard's fixed `http://localhost:5173` endpoint explicitly allowlisted. This is not shared-client coverage: Chromium does not traverse `maistro.http` | `partial` — supported Playwright transports are enumerated and guarded; a future browser integration must attach `BrowserNetworkGuard` or `SyncBrowserNetworkGuard` before it creates or receives a page |
...
1. **SSRF protection has two transport seams: ordinary HTTP and Chromium.**
   ...
   network stack connects. The supported Chromium callers are `BrowserClient`, the
   dashboard screenshot route, the async UI hill-climber, and both standalone hill-climb
   drivers (`run_hill_climb.py` and `hill-climb-ui.sh`); each attaches the sync or async
   guard before it creates a page. `tasks/progress_webhook` and `integrations/ntfy` built
   private clients and were moved onto the pool so the seam actually reaches them.
