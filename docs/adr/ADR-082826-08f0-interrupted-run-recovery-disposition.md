...
| Run `CREATED`/`QUEUED`, admitting process died | none available | **Owned by the recovery tick**: visible in `non_terminal_run_stats` and aged on the `maistro_non_terminal_runs` / `maistro_oldest_non_terminal_run_age_seconds` gauges until a sweep, a canonical consumer for its admission source, or an operator disposes of it. Silent disappearance is not a disposition. |
