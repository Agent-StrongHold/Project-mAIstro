...
| Recurrence / schedules | `services.scheduler` background loop | `none` | KEEP — converging: the cursor is canonical and durable, execution is not | ADR-082126-f69c (supersedes ADR-046) | `services/scheduler.py` advances the canonical `ScheduleStore` only after a Run exists (#231); it still executes through `run_registered_dag`, whose `DurableRunStore` is disjoint from the canonical `RunStore` — #251. `ScheduleRunAdmitter` (`maistro.scheduling.admission`) is now Container-wired (#231) though not yet the live fire path's consumer | #46, #62, #231, #251 |
...
| Workspace / Project scope | `routes.projects`, `routes.workspaces` | `none` | CONNECT — the shipped compose stack has one durable Workspace/Membership owner: Hive's embedded runtime and maistro-server share `canonical_workspaces` in maistro-engine's PostgreSQL, and Hive's `stores.workspaces` mirror is only imported once on that path. Hive's Workspace views still need a Hive presentation record, so a Workspace created only through maistro-server authorizes in Hive but is not listed there. All three Run stores validate Graph scope in `create_run`. Still open: the Turing backend's per-process `InMemoryWorkspaceStore`; `WorkspaceRoutingAdmitter` minting Root Projects for Workspace ids with no `canonical_workspaces` row; no `canonical_projects -> canonical_workspaces` FK | ADR-081226-9944, ADR-081426-b1d3, ADR-092326-97c4 | every Run carries a Project id enforced at the store boundary; every Root Project's Workspace exists in the one canonical store | #37, #38 |
...
| Model providers | `maistro.container` provider wiring | `none` | KEEP | — | provider parity tests; no new direct caller escapes | #56 |
...
| Skills, code registry, repertoire | `routes.skills`, `services.mcp_client` | `most` | MIGRATE — one governed supply-chain path (trust-policy design context in Proposed ADR-083, not shipped authority) | ADR-069, ADR-070 | signed-code verification on real register/load path | #59, #34 |
...
| Delivery gateway | none | `all` | CONNECT | — | delivery effect recorded as Invocation | #34, #57 |
...
| Authentication and identity | `routes.auth`, `middleware`, `maistro_server` auth | `none` | KEEP | ADR-077 | Argon2id registration + bcrypt upgrade | #32 |
...
| Secrets vault | `maistro.cli`, installer | `none` | KEEP | — | round-trip encryption tests | — |
...
| Reactor loop | `maistro.reactor` (installer-launched) | `none` | KEEP | — | loop timing tests | — |
| Prompts and personas | `maistro.container`, `routes.agents` | `few` | KEEP | ADR-081226-e626 | persona seed/eval protocol tests | — |
...
| Canvas ability | `maistro_canvas.canvas.routes`, `routes.canvas`, `maistro-canvas-frontend-server` | `some` | MIGRATE — pipeline stages become NodeRuns | ADR-040, ADR-067 | canvas stages visible as NodeRuns with retries as Attempts | #52 |
...
| Evolve tournament optimizer | `routes.evolution`, `services.evolution` | `few` | MIGRATE — cycle is canonical Run, battle is NodeRun; status/cycle admission is disabled or degraded without the engine Container | ADR-088 (tournament contracts Proposed in ADR-070126-6386, SPEC-070126-9d37) | tournament history reproducible from canonical Runs; `/status` distinguishes executable/degraded/unavailable and `/cycle` preserves terminal Run identity | #51, #465 |
