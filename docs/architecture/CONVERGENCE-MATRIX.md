...
| Graph execution | `services.dag_agents.run_registered_dag`; `maistro.container` node resolver | `3/65` | MIGRATE — traversal state must separate from lifecycle state | ADR-062, ADR-081226-69ee | a durable graph execution whose Run/NodeRun/Attempt records reproduce the traversal | #44, #34 |
...
| Repo tooling | the `.github/workflows/*.yml` step that executes the script | `20/61` | KEEP — the 41 rooted scripts are the gate set; the 20 unrooted are dispositioned, 10 of them behind a disabled workflow and 1 reached only through a shell installer | ADR-082526-aef8 | `scripts/check-reachability.py` roots tooling at the workflow steps that run it; `tests/test_check_reachability.py` | #33, #236, #249 |
