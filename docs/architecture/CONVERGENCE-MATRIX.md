...
| Builders pipeline | `maistro.builders` | Graph of spec→tests→code→review Nodes | `builders.runtime` + `builders.graph_executor` (fourth universal lifecycle deleted; only the canonical adapter ships) | canonical Run/NodeRun/Attempt records (`maistro.runs.store` + durable session spine) | — |
...
| Builders pipeline | `maistro.cli._builders_tui` session composition | `some` | KEEP — the shipped TUI roots the canonical adapter; the legacy in-process executor is deleted | ADR-090, ADR-099 | Builders stages appear as NodeRuns driven by `builders.graph_executor`'s canonical adapter (`CanonicalGraphPipelineExecutor`); the pre-convergence in-process executor is gone | #49, #35 |
