...
| Recurrence / schedules | `services.scheduler` background loop | `none` | KEEP — converging: the cursor is canonical and durable, execution is canonical only behind the Container bridge | ADR-082126-f69c (supersedes ADR-046) | `services/scheduler.py` advances the canonical `ScheduleStore` only after a Run exists (#231); its Graph firings go through `run_registered_dag`, which since #1113 requires the Container's canonical Run and graph-continuation stores and reports `unavailable` (leaving the occurrence owed) without them. `ScheduleRunAdmitter` (`maistro.scheduling.admission`) is now Container-wired (#231) though not yet the live fire path's consumer | #46, #62, #231, #251 |
