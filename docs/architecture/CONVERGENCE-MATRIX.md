...
| Graph execution | `services.dag_agents.run_registered_dag`; `maistro.container` node resolver | `3/61` | MIGRATE — traversal state must separate from lifecycle state | ADR-062, ADR-081226-69ee | a durable graph execution whose Run/NodeRun/Attempt records reproduce the traversal | #44, #34 |
