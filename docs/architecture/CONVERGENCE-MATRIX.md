...
| Repo tooling | the `.github/workflows/*.yml` step that executes the script | `17/50` | KEEP — the 33 rooted scripts are the gate set; the 17 unrooted are dispositioned, 10 of them behind a disabled workflow | ADR-082526-aef8 | `scripts/check-reachability.py` roots tooling at the workflow steps that run it; `tests/test_check_reachability.py` | #33, #236, #249 |
