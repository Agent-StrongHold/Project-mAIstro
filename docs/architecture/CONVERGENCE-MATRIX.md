...
| Memory | `maistro.memory` | Learning, Episode, Outcome | n/a (domain state) | `persistence.pg_learnings`/`pg_outcomes` on a `postgresql://` URL (ADR-082226-5104), `sqlite_*` on `sqlite:`, in-memory otherwise. pgvector embeddings live on the `learnings` row (ADR-082326-8194) when an embedding client is supplied | `memory.scopes`, `memory.exposure` |
...
| Relational persistence | `maistro.persistence` | Storage adapters | n/a | itself — the `pg_*` half is canonical per ADR-082226-5104 and wired for a `postgresql://` URL, prompts included (#122); the `sqlite_*` half stays for homelab | — |
...
| Repo tooling | the `.github/workflows/*.yml` step that executes the script | `16/44` | KEEP — the 28 rooted scripts are the gate set; the 16 unrooted are dispositioned, 9 of them behind a disabled workflow | ADR-082526-aef8 | `scripts/check-reachability.py` roots tooling at the workflow steps that run it; `tests/test_check_reachability.py` | #33, #236, #249 |
...
| Warden / Sentinel / Gate | `maistro.container`, `maistro_server` middleware | `10/55` | MIGRATE — construction is not enforcement | ADR-073, ADR-072, ADR-072726-0d6b | an E2E Conductor chat proving Warden/Sentinel ran on the real path | #66, #67, #68, #69, #70 |
...
| Memory | `routes.memory`, `maistro.container` | `9/24` | KEEP — domain state; align provenance. pgvector is no longer owed: `memory.vectors` and `memory.learnings.durable_hybrid` put the embedding on the row and compose similarity with scope in one query (ADR-082326-8194), reached from `create_container(embeddings=...)`. The +3 are the Archive tier's store layer (ADR-082226-d3dd), CONNECT rather than debt: written and conformance-tested against a real MinIO, not yet wired, because the policy that decides what goes cold hangs off the memory-decay path (#133) | ADR-034, ADR-011, ADR-091, ADR-057, ADR-082226-5104, ADR-082226-d3dd | a memory write that names its producing Run, with its embedding on the same row; an archived record that rehydrates byte-identical | #64, #122, #133 |
...
| Relational persistence | `maistro.container` (both backends), Alembic | `1/14` | CONNECT — only `sqlite_audit` remains unwired; #122 wired the prompt manager on every backend | ADR-082226-5104, ADR-087, ADR-012 | a container that wires `pg_*` for a `postgresql://` URL (done, #122), and pgvector embeddings on the memory tables | #122, #33, #34 |
...
| Shared contracts and config | imported by every package | `1/46` | LIBRARY | ADR-019, ADR-081226-034b | dependency-direction check; wheel-import verification | #36 |
...
- **Three subsystems own a competing work-state machine** besides `maistro.runs`, each
  ...
  *parallel run identity* rather than a state machine — its node states are folded from
  ...
  ADR-081626-f383 execution fencing — though unlike the others it is *unstarted rather than
