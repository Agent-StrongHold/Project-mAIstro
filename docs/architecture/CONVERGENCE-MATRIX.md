...
| Builders pipeline | `maistro.cli._builders_tui` session composition | `some` | KEEP — the shipped TUI roots the canonical adapter; the legacy in-process executor is deleted | ADR-090, ADR-099 | Builders stages appear as NodeRuns; `builders.graph_executor` deleted | #49, #35 |
