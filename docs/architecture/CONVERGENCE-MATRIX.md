...
| Recurrence / schedules | `services.scheduler` background loop | `none` | KEEP — converging: the cursor is canonical and durable, execution is canonical only behind the Container bridge | ADR-082126-f69c (supersedes ADR-046) | `services/scheduler.py` advances the canonical `ScheduleStore` only after a Run exists (#231). Recurring ticks and manual `fire_now` both admit through the canonical spine (#231, #1120): a manual fire claims `(schedule_id, "manual:" + fire_id)` as a first-class occurrence and admitted Runs execute via the #251 consumer tick, which manual fire triggers promptly; multi-node Runs still wait on durable Graph traversal — #44/#34. A configured-but-unwired Container fails closed (`ScheduleAdmissionUnavailable`), and with no Container at all the standalone tick's `run_registered_dag` call reports `unavailable` — since #1113 it requires the Container's canonical Run and graph-continuation stores — leaving the occurrence owed rather than executed | #46, #62, #231, #251, #1113, #1120 |
...
- Core scheduling owns occurrence identity and exact-one Run claims (#229). The **live Hive scheduler** now sends both its recurring ticks (#231) and its manual `fire_now` surface (#1120) through that seam — manual fires carry a first-class `schedule_fire_id` occurrence identity so a retried or concurrent double submit reconciles to one Run. Since #1113 there is no no-Container execution fallback: in a standalone process with no Container, `run_registered_dag` fails closed (`GraphExecutionUnavailableError`), so the tick leaves the occurrence owed rather than executing it off the canonical spine.
