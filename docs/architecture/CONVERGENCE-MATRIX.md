...
|---|---|---|---|---|---|
| Run / NodeRun / Attempt lifecycle | `maistro.runs`, `maistro.runtime` | Run, NodeRun, Attempt, ExecutionRuntime | itself (canonical) | `runs.pg_store` on a `postgresql://` URL (ADR-082226-5104), `runs.sqlite_store` on `sqlite:`, `runs.store` otherwise | caller-supplied `actor_principal_id` only |
...
| Workspace / Project scope | `maistro.workspaces`, `maistro.projects` | Workspace, Project — the scope roots | n/a (scope, not execution) | `projects.pg_scope_store` on a `postgresql://` URL, `projects.sqlite_scope_store` on `sqlite:`, `projects.scope_store` otherwise | `projects.authorization` |
...
| Warden / Sentinel / Gate | `maistro.security` | trust boundary + policy decision point | `security.strikes` / `security.pg_strikes` behind `protocols.StrikeTracker` | `security.pg_strikes` on a `postgresql://` URL, in-memory otherwise; audit log via `persistence.pg_audit` | itself (canonical) |
...
|---|---|---|---|---|---|---|
| Run / NodeRun / Attempt lifecycle | reached via `maistro.graph.durable_runs` from `services.dag_agents` | `0/18` | KEEP — canonical | ADR-081226-a66b, ADR-081426-1f7c, ADR-2026-08-16 | one conformance suite passing against the in-memory, SQLite and PostgreSQL stores, plus `formal/` property tests | #42, #43, #45 |
...
| A2A delegation | `maistro.a2a` exported API; `agent.delegate_remote` via `build_node_resolver` | `0/6` | MIGRATE — delegation must create child Runs | ADR-058 | in-process delegation admits a child Run with durable `parent_run_id`/`parent_node_run_id`; the cross-instance path needs a registered peer to exercise end to end (#147) | #47, #147 |
| Recurrence / schedules | `services.scheduler` background loop | `0/6` | KEEP — converged: a firing produces a canonical Run | ADR-082126-f69c (supersedes ADR-046) | `services/scheduler.py` fires through `run_registered_dag` and the Run carries `admission_source=schedule` with its nominal fire time; core scheduling suites. `max_runs` is still unreachable through the product surface (#145) | #46, #62, #145 |
...
| Warden / Sentinel / Gate | `maistro.container`, `maistro_server` middleware | `10/51` | MIGRATE — construction is not enforcement | ADR-073, ADR-072, ADR-072726-0d6b | an E2E Conductor chat proving Warden/Sentinel ran on the real path | #66, #67, #68, #69, #70 |
...
| Shared contracts and config | imported by every package | `1/45` | LIBRARY | ADR-019, ADR-081226-034b | dependency-direction check; wheel-import verification | #36 |
...
| maistro-server HTTP app | `maistro_server.main` | `0/19` | MIGRATE — its task lifecycle becomes a receipt | ADR-076, ADR-096 | `/v1/tasks` submission returns a canonical `run_id` | #41 |
