...
|---|---|---|---|---|---|---|
| Run / NodeRun / Attempt lifecycle | reached via `maistro.graph.durable_runs` from `services.dag_agents` | `0/20` | KEEP — canonical | ADR-081226-a66b, ADR-081426-1f7c, ADR-081626-f383 | property/conformance tests in `formal/` plus core lifecycle suites | #42, #43, #45 |
...
| Agent Conductor services | route registration + background loops | `15/62` | MIGRATE — `dag_run_store`, scheduler and graph/product seams still duplicate canonical responsibilities | ADR-096 | DAG/scheduler/chat paths use canonical Runs and projections only | #53, #35, #231 |
