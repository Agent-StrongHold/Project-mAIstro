...
| Graph execution | `maistro.graph` | Graph, Node, GraphExecutionState | `graph.durable_runs.canonical_store` over `maistro.runs` + `graph.durable_runs.continuation` (pre-durable `GraphRun`/`run_graph` RETIRED by #1154) | `graph.durable_runs.stores` (document-shaped; kept for pre-convergence records) | — |
...
| Graph execution | `services.dag_agents.run_registered_dag`; `maistro.container` node resolver | `few` | MIGRATE — traversal state must separate from lifecycle state; pre-durable `GraphRun`/`run_graph` is RETIRED (#1154) | ADR-062, ADR-081226-69ee | a durable graph execution whose Run/NodeRun/Attempt records reproduce the traversal | #44, #34 |
