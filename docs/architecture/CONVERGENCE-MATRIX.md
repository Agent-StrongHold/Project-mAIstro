...
|---|---|---|---|---|---|---|
| Run / NodeRun / Attempt lifecycle | reached via `maistro.graph.durable_runs` from `services.dag_agents` | `0/22` | KEEP — canonical | ADR-081226-a66b, ADR-081426-1f7c, ADR-081626-f383, ADR-082826-b601 | property/conformance tests in `formal/` plus core lifecycle suites | #42, #43, #45, #251 |
...
| Repo tooling | the `.github/workflows/*.yml` step that executes the script | `19/58` | KEEP — the 39 rooted scripts are the gate set; the 19 unrooted are dispositioned, 10 of them behind a disabled workflow, 1 reached only through a shell installer, and 1 the ratchet helper the gates load by file path | ADR-082526-aef8 | `scripts/check-reachability.py` roots tooling at the workflow steps that run it; `tests/test_check_reachability.py` | #33, #236, #249 |
