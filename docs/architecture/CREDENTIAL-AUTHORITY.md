...
`quality/credential-authority.json` is the authority classification. `scripts/check-credential-authority.py` joins that ledger to the same production import graph as `scripts/check-reachability.py`: every reachable credential-shaped implementation must be classified with an owner, scope, and storage contract, while retired paths must remain absent. Its behavior-shaped scan is covered by `tests/test_credential_authority.py`, including a renamed CRUD store fixture, so a new implementation must be classified and scoped before it can become reachable.
