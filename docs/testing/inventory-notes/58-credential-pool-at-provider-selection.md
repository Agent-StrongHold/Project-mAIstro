...
Net node-ID delta for `packages/maistro-core/tests`: **+8**.

- **+30** new: `tests/credentials/test_router.py` (scoped acquisition,
...
- **+4** new: `TestScopedSelection` in `tests/credentials/test_pool.py`
  (`select(allowed_key_ids=...)`).
