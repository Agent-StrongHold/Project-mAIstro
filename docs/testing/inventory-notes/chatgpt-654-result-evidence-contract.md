...
Adds fifteen focused root-suite cases for the fail-closed reusable-result contract:
...
- the CLI can complete an identity and independently verify the resulting evidence;
- completion mode refuses identity-generation arguments that would make the recorded result ambiguous;
- malformed or empty runtime metadata and malformed/noncanonical tool lists are rejected;
- malformed input entries, path ordering, and non-hex digests are rejected;
- identity schema, command, field-set, and evidence-key drift are rejected;
- malformed completed envelopes, identities, exit codes, and evidence keys are rejected;
- invalid or non-object evidence JSON fails closed; and
- CLI identity/completion modes reject missing or misplaced required arguments.
