...
The render-facade repair adds one route regression: an owned Design project returns 501 and leaves the preview service without a pending job. The three preview render-helper tests now assert the same 501 contract instead of accepting unresolvable success URLs.
