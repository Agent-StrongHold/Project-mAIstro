...
(ADR-092326-7ed7). All +21 node IDs are new, and no existing test was moved,
renamed or removed:
...
- `tests/test_default_workspace.py`: 8 node IDs. They cover first creation,
  concurrent and later calls, per-user isolation, deleted and revoked defaults,
  blank principal refusal, a durable claim lost to another process, and a
  SQLite restart leg.
