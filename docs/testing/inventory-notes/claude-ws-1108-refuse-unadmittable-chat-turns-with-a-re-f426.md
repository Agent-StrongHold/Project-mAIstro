...
- `packages/maistro-core/tests` +2: in `test_container_chat_runs.py` the two
  ...
  refusal tests, and two were added — a Run adopted with no Run store is
  refused, and a pre-dispatch `RunIntegrityError` (vetoed Attempt create) is
  refused with zero dispatches (the mutation check: restoring
  `return await dispatch()` fails it). The #338 compensation tests and the
  four no-refusal cases in `runs/test_chat_execution.py` were inverted in
