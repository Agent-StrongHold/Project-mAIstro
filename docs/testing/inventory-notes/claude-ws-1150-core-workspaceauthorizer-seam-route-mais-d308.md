...
(#1150). It adds 27 node IDs and removes none.

- Eight tests run against each of the three Workspace store backends (memory,
  SQLite, PostgreSQL), for 24 IDs: member VIEW, foreign and unknown Workspace
  denied the same way, two blank-principal cases, owner-only ADMINISTER,
  revocation after `remove_membership`, `visible_workspace_ids` matching
  `list_for_user`, and no exception chain on a denial.
