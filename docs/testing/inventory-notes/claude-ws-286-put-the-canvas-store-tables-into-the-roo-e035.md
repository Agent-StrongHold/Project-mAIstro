---
inventory-delta:
  packages/maistro-canvas/tests: +7
---
# claude-ws-286-put-the-canvas-store-tables-into-the-roo-e035

`packages/maistro-canvas/tests` gains 7 node IDs, all in the new
...
- 2 static tests, no server: every table `PgCanvasStore` names in its SQL is
  ...
  vacuously.
- 5 PostgreSQL legs (skip without `MAISTRO_TEST_PG_DSN`, fail under
  ...
  reorder, job create/claim/fenced update, composite, blob); re-applying 044
  over existing tables keeps their rows; adopting a hand-made pre-044 schema
  (no lease columns, no `canvas_blobs`); `downgrade`; and the partial
  pending-claim index.
