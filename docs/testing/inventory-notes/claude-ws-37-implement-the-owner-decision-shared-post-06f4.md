...
(#37, ADR-092326-97c4): nine node IDs in all. Three tests boot the real
...
legacy mirror import that never resurrects deleted rows. The other three
cover the fail-closed refusal when a configured database has no Container,
the unchanged no-database fallback, and the compose wiring.
