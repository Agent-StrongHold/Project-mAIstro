...
- `packages/maistro-core/tests` (+51): `workspaces/backlog/test_backlog_store_conformance.py`
  ...
  and parent cycles with diamonds that are not cycles, restart durability) plus the Goal reference and BacklogItem validation cases;
...
- `packages/maistro-server/tests` (+9): `api/test_backlog_api.py` covers member
  ...
  fields and duplicate external keys, fail-closed without a store, and mounting on
  the production app.
