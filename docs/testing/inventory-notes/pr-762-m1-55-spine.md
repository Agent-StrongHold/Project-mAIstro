...
added 11 net node IDs, and the follow-up logical identity regression test added
one more, for 12 net node IDs in
...
- `tests/graph/nodes/test_agent_spawn_harness.py`: dispatch-guard arcs
  ...
  handle) plus governed-path coverage and distinct-request effect-key
  isolation.
...
No suite silently stopped collecting — every one of the 12 IDs is a test
