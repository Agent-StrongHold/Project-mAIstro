...
class _NoopSink:
    """Audit sink that accepts every entry — the trail itself is what this
    model tracks, not the sink's durability."""
...
class RollbackConformanceMachine(RuleBasedStateMachine):
    ...
    def __init__(self):
        ...
        self.trail = GenomeAuditTrail(_NoopSink())
        self.next_id = 0
    ...
    def regression_signal_triggers_rollback(self):
        result = self._run(self.store.rollback_audited(self.trail))
        if result is None:
            return
    ...
    def double_rollback_is_safe(self):
        ...
        first = self._run(self.store.rollback_audited(self.trail))
        if first is not None and self.promotion_history:
            self.promotion_history.pop()
        second = self._run(self.store.rollback_audited(self.trail))
...
def test_rollback_always_restores_the_immediately_prior_promotion(promotion_count, rollback_count):
    ...
    for _ in range(rollback_count):
        result = asyncio.run(store.rollback_audited(trail))
        if result is None:
...
def test_rollback_with_nothing_promoted_yet_is_a_safe_noop():
    store = PopulationStore()
    trail = GenomeAuditTrail(_NoopSink())
    assert asyncio.run(store.rollback_audited(trail)) is None
