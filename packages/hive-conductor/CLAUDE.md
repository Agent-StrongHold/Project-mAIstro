...
The backend uses the explicit `hive_conductor` application namespace. Import the shipped ASGI app as
`hive_conductor.main:app`; routes, middleware, config, and services stay below that package so the app can
run beside the Turing backend without generic module collisions.
...
uvicorn hive_conductor.main:app --reload --port 8101
```

- Entry: `hive_conductor.main:app` on port **8101**. Routes are registered in `hive_conductor/main.py`; handlers are under `hive_conductor/routes/`.
...
```
The root pytest configuration exposes `backend/` as a package root; the suite does not rewrite `sys.path`.
Its conftest stubs the engine/foundation singletons, seeds test users, and provides `authed_client` /
`admin_client` fixtures (auto-login). No manual PYTHONPATH needed.
