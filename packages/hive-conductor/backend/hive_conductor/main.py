...
from hive_conductor.logging_setup import configure_logging
...
from hive_conductor.middleware.request_log import RequestLogMiddleware
from hive_conductor.middleware.security_headers import SecurityHeadersMiddleware
from hive_conductor.routes import (
    agents,
    audit,
    auth,
    capabilities,
...
from hive_conductor.routes import metrics as metrics_r
from hive_conductor.routes import optimizer as optimizer_r
from hive_conductor.routes import settings as settings_r
from hive_conductor.services import engine as engine_service
from hive_conductor.services import foundation as foundation_service
from hive_conductor.services.ha_tools import get_all_confirms, get_pending_confirms, respond_confirm
from hive_conductor.services.oauth_login import close_oauth_login_service
from hive_conductor.services.settings_store import SettingsPersistenceError
from maistro.observability.middleware import REQUEST_ID_HEADER, RequestIDMiddleware

# The package lives at /app/backend/hive_conductor in the image; the SPA is
# copied to /app/frontend, so resolve the image root rather than the package.
...
async def _shutdown_background_services() -> None:
    ...
    stoppers: tuple[tuple[str, str], ...] = (
        ("hive_conductor.services.design_service", "stop_design_service"),
        ("hive_conductor.services.evolution", "stop_evolution"),
        ("hive_conductor.services.scheduler", "stop_scheduler"),
        ("hive_conductor.services.memory_decay", "stop_memory_decay"),
    )
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        ...
        from hive_conductor.services.agent_materialization import materialize_boot_roster
    ...
    try:
        from hive_conductor.services.design_service import start_design_service

        await start_design_service(get_settings())
        from hive_conductor.services.design_preview import init_design_preview_service
    ...
    try:
        from hive_conductor.services.scheduler import start_scheduler
    ...
    try:
        ...
        from hive_conductor.services.memory_decay import start_memory_decay

        await start_memory_decay(get_settings())
    except Exception as exc:
        _lifespan_log.warning("memory_decay_start_failed: %s", exc, exc_info=True)
    try:
        from hive_conductor.services.evolution import start_evolution
...
def create_app() -> FastAPI:
    ...
    _include_optional_router(app, "hive_conductor.routes.design", prefix="/v1")
    _include_optional_router(app, "hive_conductor.routes.canvas")
    _include_optional_router(app, "hive_conductor.routes.evolution", prefix="/v1/evolution")
    _include_optional_router(app, "hive_conductor.routes.rsi", prefix="/v1/rsi")
