...
from hive_conductor.adapters.task_backend import WORKSPACE_NOT_ROUTABLE_DETAIL, WorkspaceNotRoutable
from hive_conductor.models.schemas import Mission, MissionStep
from hive_conductor.routes.audit import log_audit
from hive_conductor.services.engine import get_engine
from hive_conductor.services.workspace_mode import is_workspace_member
...
def _revoke_task_elevation(request: Request, task_id: str) -> None:
    ...
    if not session_id:
        return
    try:
        from hive_conductor.routes.auth import revoke_task_elevation
        ...
        __import__("logging").getLogger("hive.routes.missions").warning(
            ...
            "packages/hive-conductor/backend/hive_conductor/routes/missions.py",
            193,
            _exc,
        )
        pass
...
async def post_mission_guidance(
    ...
    from hive_conductor.services.program_hyperagent import (
