...
from hive_conductor.protocols.llm import LLMPort
...
from hive_conductor.services.airtable_cache import (
...
def _build_system_prompt(user_id: str) -> str:  # noqa: C901  many optional prompt sections
    ...
    from hive_conductor.services import profile_store
...
async def _tool_save_as_action(
    ...
    from hive_conductor.models.schemas import Agent as AgentModel
...
async def _tool_web_search(
    ...
    try:
        from hive_conductor.services.tool_executor import web_search as _web_search
...
async def _tool_browse_url(
    ...
    try:
        from hive_conductor.services.tool_executor import browse_url as _browse_url
...
async def _tool_create_dashboard_widget(
    ...
    *reported*, not silently dropped, so the model can correct the config
    ...
    from hive_conductor.services.dashboard_safety import widget_config_violations
...
from hive_conductor.services.substrate_tools import (  # noqa: E402
    tool_hill_climb,
    tool_mutate_workflow,
)
...
async def _run_chat_completion_inner(
    ...
    if not inbound.allowed:
        from hive_conductor.services.chat_gate import openai_refusal
...
if os.environ.get("DEPLOY_TARGET_APP_ENV"):
    ...
    async def _pg_memory_add(args, user_id, jira_pat=None):
        ...
        if result.get("saved") and result.get("id"):
            try:
                from hive_conductor.services.pg_store import pg_upsert
    ...
    async def _pg_memory_delete(args, user_id, jira_pat=None):
        ...
        if result.get("deleted"):
            try:
                from hive_conductor.services.pg_store import pg_delete
