...
def _settings() -> Any:
    ...
    the wrong instrument regardless: the import is deferred so lightweight
    policy tooling can use this module without initializing application
    settings at process start. Every read of the result already goes through
    `getattr(..., default)`, so `Any` is what the
