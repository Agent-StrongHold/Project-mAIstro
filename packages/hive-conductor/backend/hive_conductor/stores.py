...
def _seed_platform_mcp() -> None:
    ...
    from hive_conductor.services.mcp_defaults import merge_manifest_catalog, platform_mcp_catalog
...
def _seed_if_empty() -> None:
    _seed_platform_mcp()
    from hive_conductor.settings_defaults import is_pm_poc_mode
...
def _seed_dags() -> None:
    if len(dags) == 0:
        from hive_conductor.routes.dags import DAGEdge, DAGFile, DAGNode
...
def _seed_messages() -> None:
    if len(messages) == 0:
        from hive_conductor.routes.messages import Message
