...
async def list_visible_runs(user_id: str, *, limit: int = 25) -> list[dict[str, Any]]:
    ...
    # Read the whole retained projection window before applying the caller's
    # limit; foreign rows must not hide an older in-scope run.
    projections = {row["id"]: row for row in history.list_runs(limit=MAX_RUNS)}
