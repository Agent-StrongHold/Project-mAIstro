    ...
    <div style={{ display: logsFor ? "grid" : "block", gridTemplateColumns: logsFor ? "1fr 420px" : "1fr", gap: 0, minHeight: "calc(100vh - 60px)" }}>
      <div>
        <PageHeader
          ...
          actions={<button className="btn" style={{ fontSize: 12, padding: "2px 8px" }} onClick={() => void load()}>refresh</button>}
        />
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 8 }}>
            ...
            <label style={{ display: "flex", gap: 4, alignItems: "center", fontFamily: "var(--mono)", fontSize: 12, color: "var(--pencil)", cursor: "pointer" }}>
                ...
                <div key={c.id} className="card" style={{ opacity: isActing ? 0.6 : 1 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--pencil)", marginTop: 2 }}>{c.image}</div>
                    </div>
                    <div style={{ display: "flex", gap: 3 }}>
                      {isRunning ? (
                        <>
                          <button className="btn" style={{ fontSize: 12, padding: "1px 6px" }} onClick={() => void containerAction(c.id, "restart")} disabled={isActing}>restart</button>
                          <button className="btn" style={{ fontSize: 12, padding: "1px 6px", borderColor: "var(--warn)", color: "var(--warn)" }} onClick={() => void containerAction(c.id, "stop")} disabled={isActing}>stop</button>
                          <button className="btn" style={{ fontSize: 12, padding: "1px 6px" }} onClick={() => void fetchLogs(c.id, c.name)}>logs</button>
                        </>
                      ) : (
                        <>
                          <button className="btn" style={{ fontSize: 12, padding: "1px 6px", borderColor: "var(--ok)", color: "var(--ok)" }} onClick={() => void containerAction(c.id, "start")} disabled={isActing}>start</button>
                          <button className="btn" style={{ fontSize: 12, padding: "1px 6px", borderColor: "var(--danger)", color: "var(--danger)", opacity: 0.6 }} onClick={() => void removeContainer(c.id)} disabled={isActing}>remove</button>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  {isRunning && c.memory_limit_mb > 0 && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--pencil)", marginBottom: 2 }}>MEMORY {memPct.toFixed(0)}%</div>
                  ...
                  <div style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--pencil)", marginTop: 6 }}>
        ...
        <div style={{ borderLeft: "1.5px dashed var(--honey)", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "8px 0 6px 10px" }}>
            <div>
              ...
              <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--pencil)", marginLeft: 8 }}>logs</span>
            </div>
            <div style={{ display: "flex", gap: 4 }}>
              <button className="btn" style={{ fontSize: 12, padding: "1px 6px" }} onClick={() => void fetchLogs(logsFor.id, logsFor.name)}>refresh</button>
              <span className="btn" style={{ fontSize: 12, padding: "1px 6px" }} onClick={() => { setLogsFor(null); setLogs(""); }}>close</span>
            </div>
          </div>
          <div style={{ flex: 1, margin: "0 0 0 10px", background: "var(--ink)", color: "var(--paper)", fontFamily: "var(--mono)", fontSize: 12, padding: "10px 12px", borderRadius: 4, overflow: "auto", maxHeight: "calc(100vh - 140px)", whiteSpace: "pre-wrap", wordBreak: "break-all", lineHeight: 1.6 }}>
