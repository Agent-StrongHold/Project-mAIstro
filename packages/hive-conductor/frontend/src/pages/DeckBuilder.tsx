...
function DeckChat({ slides, onUpdateSlides, activeIdx }: { slides: Slide[]; onUpdateSlides: (s: Slide[]) => void; activeIdx: number }) {
  ...
  return <section aria-labelledby="deck-chat-title" style={{ borderTop: `1px solid ${C.border}`, marginTop: "1rem", paddingTop: "0.75rem" }}>
    <h2 id="deck-chat-title" style={{ fontSize: "0.65rem", color: C.muted, margin: "0 0 6px", textTransform: "uppercase" }}>Slide assistant</h2>
    {msgs.length > 0 && <div ref={ref} aria-label="Assistant messages" style={{ maxHeight: 150, overflowY: "auto", marginBottom: 8, display: "flex", flexDirection: "column", gap: 4 }}>{msgs.slice(-6).map((message, index) => <div key={index} style={{ fontSize: "0.65rem", color: message.role === "user" ? C.gold : C.muted, lineHeight: 1.4 }}><span style={{ fontWeight: 600 }}>{message.role === "user" ? "You" : "Assistant"}: </span>{message.content.replace(/<slide[^>]*>[\s\S]*?<\/slide>/gi, "[slide generated]").slice(0, 200)}</div>)}</div>}
    <div role="status" aria-live="polite" style={{ color: C.muted, fontSize: "0.65rem", marginBottom: 6 }}>{status}</div>
    <div style={{ display: "flex", gap: 6 }}><label htmlFor="deck-assistant-input" style={{ position: "absolute", width: 1, height: 1, overflow: "hidden" }}>Describe slide changes</label><input id="deck-assistant-input" value={value} onChange={(event) => setValue(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void submit(); }} placeholder="Describe slides to generate, or ask to edit..." style={{ flex: 1, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 12px", color: C.ink, fontSize: "0.72rem" }} /><button type="button" onClick={() => void submit()} disabled={loading}>{loading ? "Generating..." : "Generate"}</button></div>
  </section>;
}
...
export default function DeckBuilder({ initialPrompt = "", onExit, onSave }: DeckBuilderProps) {
  ...
  useEffect(() => {
    if (!presenting) presentationReturnRef.current?.focus();
  }, [presenting]);
  ...
  function startPresentation(element: HTMLElement): void { presentationReturnRef.current = element; setPresentationSlide(active); setPresenting(true); }
