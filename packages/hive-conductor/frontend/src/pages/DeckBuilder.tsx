...
function safeSlide(slide: Slide): Slide { return { ...slide, html: sanitizeVisualArtifactMarkup(slide.html) }; }
const BLANK_SLIDE = (prompt = ""): Slide => ({ id: uid(), html: `<h1>${escapeVisualArtifactText(prompt || "Title")}</h1><p>Content</p>`, notes: "" });
...
export default function DeckBuilder({ initialPrompt = "", onExit, onSave }: DeckBuilderProps) {
  ...
  const updateSlide = useCallback((index: number, html: string) => setSlides((current) => current.map((slide, i) => i === index ? { ...slide, html: sanitizeVisualArtifactMarkup(html) } : slide)), []);
  const insertPreviewContent = useCallback((target: HTMLDivElement, raw: string, asHtml: boolean) => {
    const fragment = document.createDocumentFragment();
    if (asHtml) { const template = document.createElement("template"); template.innerHTML = sanitizeVisualArtifactMarkup(raw); fragment.appendChild(template.content); } else fragment.appendChild(document.createTextNode(raw));
  ...
  function exportHTML(): void { const safeTitle = escapeVisualArtifactText(title); const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${safeTitle}</title></head><body><main>${slides.map((slide) => `<section>${sanitizeVisualArtifactMarkup(slide.html)}</section>`).join("\n")}</main></body></html>`; const url = URL.createObjectURL(new Blob([html], { type: "text/html" })); const link = document.createElement("a"); link.href = url; link.download = `${title.replace(/[^a-z0-9._-]+/gi, "-").replace(/^-+|-+$/g, "") || "deck"}.html`; link.click(); URL.revokeObjectURL(url); setStatus("Deck exported as HTML."); }

  if (presenting) return <div ref={presentationRef} role="dialog" aria-modal="true" aria-labelledby="presentation-title" style={{ position: "fixed", inset: 0, background: C.bg, zIndex: 9999, color: C.ink, overflow: "hidden", padding: 16 }}><h1 id="presentation-title" style={{ position: "absolute", width: 1, height: 1, overflow: "hidden" }}>{title} presentation</h1><div style={{ height: "calc(100vh - 70px)", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ width: "min(100%, 1000px)", maxHeight: "100%", aspectRatio: "16/9", overflow: "auto", padding: "4rem", boxSizing: "border-box" }} dangerouslySetInnerHTML={{ __html: sanitizeVisualArtifactMarkup(slides[presentationSlide]?.html || "") }} /></div><div style={{ position: "fixed", bottom: 12, left: 12, right: 12, display: "flex", justifyContent: "center", gap: 8, alignItems: "center" }}><button type="button" onClick={() => setPresentationSlide((current) => Math.max(0, current - 1))} disabled={presentationSlide === 0}>Previous slide</button><span aria-live="polite">Slide {presentationSlide + 1} of {slides.length}</span><button type="button" onClick={() => setPresentationSlide((current) => Math.min(slides.length - 1, current + 1))} disabled={presentationSlide === slides.length - 1}>Next slide</button></div><button ref={presentationExitRef} type="button" onClick={() => setPresenting(false)} style={{ position: "fixed", top: 12, right: 12 }}>{"Exit (Esc)"}</button></div>;
