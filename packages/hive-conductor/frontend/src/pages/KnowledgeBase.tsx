  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.ink, fontFamily: "'Inter Variable', 'Inter', -apple-system, system-ui, sans-serif", padding: "1.5rem 2rem" }}>
      <div style={{ marginBottom: "1rem" }}>
        ...
        <p style={{ fontSize: "var(--text-floor)", color: C.muted, margin: "2px 0 0" }}>Your personal chamber — identity, preferences, and everything Fantasia knows about you</p>
      </div>

      {loadError && (
        <div role="alert" style={{ background: "rgba(232,124,124,0.1)", border: `1px solid ${C.danger}`, borderRadius: 8, padding: "8px 12px", marginBottom: "0.75rem", fontSize: "var(--text-floor)", color: C.danger }}>
      ...
      {!profileDurable && (
        <div style={{ background: "rgba(196,166,97,0.08)", border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 12px", marginBottom: "0.75rem", fontSize: "var(--text-floor)", color: C.gold }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "10px 14px", marginBottom: "1rem" }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {chatLoading && <span style={{ fontSize: "var(--text-floor)", color: C.muted }}>…</span>}
        </div>
        {chatHistory.length > 0 && (
          <div style={{ marginTop: 8, maxHeight: 160, overflowY: "auto", display: "flex", flexDirection: "column-reverse", gap: 5 }}>
            {[...chatHistory].reverse().map((m, i) => (
              <div key={i} style={{ padding: "5px 10px", borderRadius: 8, background: m.role === "user" ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.3)", fontSize: "var(--text-floor)", color: C.ink, lineHeight: 1.4, whiteSpace: "pre-wrap" }}>
                <span style={{ fontSize: "var(--text-floor)", color: C.muted, marginRight: 5 }}>{m.role === "user" ? "You" : "✦"}</span>{m.content}
              </div>
            ))}
            <button onClick={() => setChatHistory([])} style={{ alignSelf: "flex-end", background: "none", border: "none", color: C.dim, cursor: "pointer", fontSize: "var(--text-floor)" }}>clear</button>
          </div>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 2, marginBottom: "1rem", borderBottom: `1px solid ${C.border}`, paddingBottom: 2 }}>
        {([["prompts", "Prompts"], ["profile", `Profile (${filledFields}/${totalFields})`], ["memories", `Memories (${entries.length})`], ["settings", "Settings"]] as [Tab, string][]).map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: "7px 14px", borderRadius: "8px 8px 0 0", border: "none", cursor: "pointer", fontSize: "var(--text-floor)", fontWeight: 600, background: tab === t ? "rgba(167,139,250,0.12)" : "transparent", color: tab === t ? C.ink : C.muted, borderBottom: tab === t ? `2px solid ${C.acc}` : "2px solid transparent" }}>
            {label}
          </button>
        ))}
      </div>
          ...
          {PROMPT_SECTIONS.map(s => (
            <div key={s.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div>
                  <div style={{ fontSize: "var(--text-floor)", fontWeight: 600, color: C.ink }}>{s.label}</div>
                  <div style={{ fontSize: "var(--text-floor)", color: C.muted }}>{s.desc}</div>
                </div>
              </div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: "var(--text-floor)", color: C.dim, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>System Prompt (admin only)</div>
                <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: 6, padding: "8px 10px", fontSize: "var(--text-floor)", color: C.muted, lineHeight: 1.4, maxHeight: 80, overflowY: "auto", whiteSpace: "pre-wrap" }}>
                  {prompts[s.id]?.system || "(default — managed by system)"}
                </div>
              </div>
              <div>
                <div style={{ fontSize: "var(--text-floor)", color: C.gold, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>Your additions (editable)</div>
                <textarea value={prompts[s.id]?.user || ""} onChange={e => { const v = e.target.value; setPrompts(p => ({ ...p, [s.id]: { ...p[s.id], user: v } })); }}
                  ...
                  style={{ width: "100%", minHeight: 60, border: `1px solid ${C.border}`, background: "rgba(0,0,0,0.2)", borderRadius: 6, padding: "8px 10px", color: C.ink, fontSize: "var(--text-floor)", resize: "vertical", outline: "none", fontFamily: "inherit", lineHeight: 1.4 }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {Object.entries(PROFILE_SECTIONS).map(([key, section]) => (
            <div key={key} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 14px" }}>
              <div style={{ fontSize: "var(--text-floor)", fontWeight: 600, color: C.gold, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>{section.label}</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "4px 16px" }}>
                {section.fields.map(f => (
                  <div key={f} style={{ fontSize: "var(--text-floor)", padding: "2px 0" }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: 10, marginBottom: 10 }}>
              <textarea value={newContent} onChange={e => setNewContent(e.target.value)} placeholder="Add a memory..." rows={2}
                style={{ width: "100%", border: "none", background: "transparent", color: C.ink, fontSize: "var(--text-floor)", resize: "vertical", outline: "none", fontFamily: "inherit" }} />
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 4 }}>
                <input value={newTags} onChange={e => setNewTags(e.target.value)} placeholder="tags" style={{ flex: 1, border: `1px solid ${C.border}`, background: "transparent", borderRadius: 6, padding: "3px 8px", color: C.ink, fontSize: "var(--text-floor)", outline: "none" }} />
                <button onClick={addEntry} disabled={!newContent.trim()} style={{ padding: "3px 10px", borderRadius: 6, border: "none", background: C.acc, color: "#fff", fontSize: "var(--text-floor)", fontWeight: 600, cursor: "pointer", opacity: newContent.trim() ? 1 : 0.4 }}>Save</button>
              </div>
            </div>
            {loading ? <div style={{ color: C.muted, fontSize: "var(--text-floor)" }}>Loading...</div> :
              entries.length === 0 ? <div style={{ color: C.muted, fontSize: "var(--text-floor)" }}>No memories yet.</div> :
              entries.map(e => (
                <div key={e.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 10px", marginBottom: 6 }}>
                  <div style={{ fontSize: "var(--text-floor)", color: C.ink, lineHeight: 1.4 }}>{e.value}</div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
                    <div style={{ display: "flex", gap: 3 }}>
                      <span style={{ fontSize: "var(--text-floor)", padding: "1px 5px", borderRadius: 4, background: "rgba(167,139,250,0.1)", color: C.acc }}>{e.namespace}</span>
                      {e.tags?.map(t => <span key={t} style={{ fontSize: "var(--text-floor)", padding: "1px 5px", borderRadius: 4, background: "rgba(196,166,97,0.08)", color: C.gold }}>{t}</span>)}
                    </div>
                    <div style={{ display: "flex", gap: 3 }}>
                      <button onClick={() => reinforceEntry(e.id)} style={{ background: "none", border: "none", color: C.ok, cursor: "pointer", fontSize: "var(--text-floor)" }}>↑</button>
                      <button onClick={() => deleteEntry(e.id)} style={{ background: "none", border: "none", color: C.danger, cursor: "pointer", fontSize: "var(--text-floor)" }}>✕</button>
                    </div>
                  </div>
                </div>
              ))
      ...
      {tab === "settings" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px" }}>
            <div style={{ fontSize: "var(--text-floor)", fontWeight: 600, color: C.gold, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>Model Preferences</div>
            {profile.favorite_models && (
              <div style={{ marginBottom: 8 }}>
                <span style={{ fontSize: "var(--text-floor)", color: C.muted }}>Favorites: </span>
                <span style={{ fontSize: "var(--text-floor)", color: C.ink }}>{(profile.favorite_models as unknown as string[] || []).join(", ") || "—"}</span>
              </div>
            )}
            {profile.task_models && (
              <div style={{ marginBottom: 8 }}>
                <span style={{ fontSize: "var(--text-floor)", color: C.muted }}>Per-task: </span>
                {Object.entries(profile.task_models as unknown as Record<string, string> || {}).map(([t, m]) => (
                  <span key={t} style={{ fontSize: "var(--text-floor)", color: C.ink, marginRight: 10 }}>{t.replace(/_/g, " ")}: {m}</span>
                ))}
                {!Object.keys(profile.task_models || {}).length && <span style={{ fontSize: "var(--text-floor)", color: C.dim }}>—</span>}
              </div>
            )}
            {profile.hidden_models && (profile.hidden_models as unknown as string[] || []).length > 0 && (
              <div style={{ marginBottom: 8 }}>
                <span style={{ fontSize: "var(--text-floor)", color: C.muted }}>Hidden: </span>
                <span style={{ fontSize: "var(--text-floor)", color: C.dim }}>{(profile.hidden_models as unknown as string[]).join(", ")}</span>
              </div>
            )}
            <p style={{ fontSize: "var(--text-floor)", color: C.dim, margin: "8px 0 0" }}>Use chat: "favorite gpt-5.5" · "hide cerebras" · "use claude for widget wizard"</p>
          </div>
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: "14px 16px" }}>
            <div style={{ fontSize: "var(--text-floor)", fontWeight: 600, color: C.gold, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>System</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: "6px 20px" }}>
              {[["default_model", "Default Model"], ["fallback_model", "Fallback Model"], ["max_tokens", "Max Tokens"], ["stream_responses", "Streaming"], ["theme", "Theme"]].map(([key, label]) => (
                <div key={key} style={{ fontSize: "var(--text-floor)", padding: "3px 0", display: "flex", justifyContent: "space-between" }}>
