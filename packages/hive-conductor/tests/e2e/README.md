...
| `pm-workflow.spec.ts` | **Yes** | Playwright UI test. `ci.yml`'s `hive-conductor-e2e-ui` job runs `docker-compose.test.yml`'s `e2e-tests` service, which builds `tests/Dockerfile.playwright` and runs every `*.spec.ts` in this directory. (This row said "not wired yet" until #377; the job had been added in the meantime.) |
