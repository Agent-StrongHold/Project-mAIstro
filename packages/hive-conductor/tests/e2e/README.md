...
| `optimistic-mutations.spec.ts` | **Yes** | Same job, same config. Toggling a schedule and creating/deleting a memory entry patch local state from the mutation's own response instead of refetching the whole collection afterward, matching the pattern `WorkspaceContext.tsx` already used for workspace archive/delete (#1422). |
