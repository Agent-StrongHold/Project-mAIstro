...
class CanvasStore(Protocol):
    ...
    ``org_id`` and must carry it into the database predicate (#857): a
    global fetch filtered in Python is not an authorized read. Rows in
    another org read as absent — ``None`` / ``NotFound`` — so existence
    itself stays scoped information.
    """
