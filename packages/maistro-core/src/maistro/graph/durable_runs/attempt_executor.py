...
from maistro.graph.nodes.base import ChildNodeOverride, NodeContext, NodeResult
...
def _composed_child_resolver(
    node_override: ChildNodeOverride | None,
    node_resolver: NodeResolver,
) -> NodeResolver:
    """Prefer a composite node's private adapter, then fall through to production wiring."""

    def resolve(child_node_id: str, graph: Graph) -> Any:
        if node_override is not None:
            overridden = node_override(child_node_id, graph)
            if overridden is not None:
                return overridden
        return node_resolver(child_node_id, graph)

    return resolve


def _child_blackboard_metadata(
    record: DurableRunRecord,
    *,
    parent_node_type: str,
) -> dict[str, Any]:
    metadata = dict(record.graph_state.blackboard_snapshot.get("metadata") or {})
    if parent_node_type == "agent.synth_dag":
        metadata["synth_depth"] = int(metadata.get("synth_depth", 0)) + 1
    return metadata


async def _launch_child_graph(
    child_graph: Any,
    ...
    *,
    ...
    """Launch one composite child Graph through the same canonical execution spine."""
    ...
    return await run_durable_graph(
        child_graph,
        store=store,
        node_resolver=_composed_child_resolver(node_override, node_resolver),
        inputs=child_inputs,
        ...
        blackboard_metadata=_child_blackboard_metadata(
            record,
...
async def _execute_one(
    ...
    *,
    ...
    """Execute/recover one frontier member through a canonical Attempt."""
    ...
    async def run_child_graph(
        ...
        return await _launch_child_graph(
            child_graph,
            child_inputs,
            node_override,
            record=record,
            parent_node_id=node_id,
            parent_node_type=str(spec.node_type),
