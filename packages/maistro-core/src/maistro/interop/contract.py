...
class InteropOntology:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "relationships",
            MappingProxyType(dict(self.relationships)),
        )
        ...
        for name, spec in self.relationships.items():
            ...
            for field_name, cardinality in (
                ...
                if cardinality not in _CARDINALITIES:
                    raise InteropContractError(
                        f"relationship {name!r} has invalid {field_name} "
                        f"{cardinality!r}"
                    )
    ...
    def validate_projection(self, concept: str, projection: Mapping[str, object]) -> str:
        ...
        if spec.revision is not None:
            ...
            if not valid_string and not valid_integer:
                raise InteropContractError(
                    f"{concept} projection must carry canonical revision field "
                    f"{spec.revision!r}"
                )
        return value
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "relationships": {
                name: spec.as_dict() for name, spec in self.relationships.items()
            },
