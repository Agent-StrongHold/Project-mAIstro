...
def classify(attempt: Attempt, node_run: NodeRun) -> Finding:
    ...
    if outcome is None or outcome.attempt_result.attempt_id != attempt.attempt_id:
    ...
    elif node_run.result is None or evidence_values_equal(node_run.result, attempt.result):
        ...
        # There is nothing here the Attempt does not already have.
...
async def survey(store: RunStore, *, limit: int = DEFAULT_SWEEP_LIMIT) -> Survey:
