...
claims table is provisioned at wire time (``ensure_schema``) — the alembic
chain (038 base, 039 fencing) need not have run yet — and a provisioning
failure there fails the wiring
...
class PgTaskIdempotencyStore(_ClaimFlow):
    ...
    async def ensure_schema(self) -> None:
        ...
        The alembic chain creates this table through migrations 038 (base)
        and 039 (fencing), but wiring does not wait for it: a spine-ready
        pool whose chain has not run yet gets the table right here (the same
        self-provisioning the SQLite tier has always done), so a restart
        reconciles retries instead of minting a second Run. The DDL mirrors
        the migrations' column set exactly, so the later ``alembic upgrade
        head`` finds the shape it expects (039 adopts a provisioned fenced
        table; it evolves a chain-built 038 table in place). A pool
...
async def wire_task_idempotency(conn: Any, *, pg_pool: Any = None) -> TaskIdempotencyStore:
    ...
    - **Spine-ready pool → durable claims.** ``ensure_schema`` provisions the
      table at wiring time, so the alembic chain (038 base, 039 fencing)
      need not have run yet. A
