...
The layer is no longer specification-only. `agent.spawn_harness` and ordinary
Agent model calls resolve a scoped Binding and cross the governed Invocation
boundary before a provider-specific physical effect. These tests pin that reach
and the durable store/migration distinction from the event-handler store.
"""
...
class TestTheStoreStatesItsReachAndItsTable:
    ...
    def test_the_capability_store_is_distinct_from_event_handler_storage(self) -> None:
        assert "maistro.events.invocations" in (invocation_store.__doc__ or "")

    @pytest.mark.ac("SPEC-083026-6cef/AC-2")
    def test_the_capability_store_has_a_durable_migration(self) -> None:
        creating = [
            path.name
            for path in sorted(_MIGRATIONS.glob("*.py"))
            if "capability_invocations" in path.read_text()
        ]
        assert creating == ["033_canonical_invocation_quota.py"]
