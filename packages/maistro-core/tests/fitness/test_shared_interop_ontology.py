...
_EXPECTED_LINEAGE = tuple(
    [*zip(_EXECUTION_CHAIN, _EXECUTION_CHAIN[1:]), *zip(_EFFECT_CHAIN, _EFFECT_CHAIN[1:])]
)
...
_EXPECTED_CONCEPTS: dict[str, dict[str, str]] = {
    ...
    "Graph": {"owner": "maistro.graph", "identity": "graph_id", "scope": "Project"},
    "Run": {
        "owner": "maistro.runs",
        "identity": "run_id",
        "scope": "Project",
    ...
    "NodeRun": {"owner": "maistro.runs", "identity": "node_run_id", "parent": "Run"},
    "Attempt": {
        "owner": "maistro.runs",
        "identity": "attempt_id",
        "parent": "NodeRun",
    ...
    "Capability": {"owner": "maistro.capabilities", "identity": "capability_id"},
    "Provider": {
...
def _contract_failures(contract: Mapping[str, Any]) -> list[str]:
    ...
    if not isinstance(principles, list) or not _REQUIRED_PRINCIPLES.issubset(set(principles)):
    ...
    for name, expected in _EXPECTED_CONCEPTS.items():
        ...
        if actual != expected:
            failures.append(f"{name} canonical metadata drifted: expected {expected!r}, got {actual!r}")
    ...
    for parent, child in edges:
        ...
        if child_metadata.get("parent") != parent and child_metadata.get("scope") != parent:
            failures.append(
                f"lineage edge {parent} -> {child} disagrees with {child} parent/scope metadata"
            )

    graph_state = concepts.get("GraphExecutionState")
    run = concepts.get("Run")
    if isinstance(graph_state, dict) and isinstance(run, dict):
        if graph_state.get("identity") != run.get("identity") or graph_state.get("parent") != "Run":
            failures.append("GraphExecutionState must remain traversal state keyed by the Run identity")
...
def _documentation_failures(contract: Mapping[str, Any], text: str) -> list[str]:
    ...
    if rows != expected_rows:
        failures.append(
            ...
            f"missing_or_changed={sorted(set(expected_rows.items()) - set(rows.items()))!r} "
            f"unexpected={sorted(set(rows.items()) - set(expected_rows.items()))!r}"
        )
...
def test_validator_rejects_canonical_owner_and_identity_drift() -> None:
    ...
    assert any("Run canonical metadata drifted" in failure for failure in _contract_failures(owner_drift))

    identity_drift = _load_contract()
    identity_drift["concepts"]["Run"]["identity"] = "builder_run_id"
    assert any(
        "Run canonical metadata drifted" in failure for failure in _contract_failures(identity_drift)
    )
...
def test_validator_rejects_duplicate_lineage_edge() -> None:
    ...
    assert any("duplicate edges" in failure for failure in _contract_failures(contract))
...
def test_validator_rejects_missing_required_consumer() -> None:
    ...
    assert any("consumer conductor" in failure for failure in _contract_failures(contract))


def test_validator_rejects_documentation_drift() -> None:
    contract = _load_contract()
    text = _DOC_PATH.read_text(encoding="utf-8")
    drifted = text.replace("`maistro.runs` | `run_id`", "`maistro.builders` | `builder_run_id`", 1)
