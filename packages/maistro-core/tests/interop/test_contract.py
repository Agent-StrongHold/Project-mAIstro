...
def test_projection_requires_the_canonical_identity_field() -> None:
    ...
    # The package also exports the equivalent module-level form; both shapes
    # are the reviewed public API and must agree.
    with pytest.raises(
        InteropContractError,
        match="Run projection must carry canonical identity field 'run_id'",
    ):
        validate_projection(INTEROP_ONTOLOGY_V1, "Run", {"graph_id": "graph-1"})

    assert (
        validate_projection(
            INTEROP_ONTOLOGY_V1,
            "Run",
            {"run_id": "run-1", "graph_id": "graph-1"},
        )
        == "run-1"
    )
...
def test_major_version_drift_fails_loudly() -> None:
    require_compatible(INTEROP_ONTOLOGY_V1, "1.9.0")

    with pytest.raises(InteropContractError, match="incompatible"):
        require_compatible(INTEROP_ONTOLOGY_V1, "2.0.0")

    with pytest.raises(InteropContractError, match=r"semantic x\.y\.z"):
        require_compatible(INTEROP_ONTOLOGY_V1, "v1")
    ...
    with pytest.raises(
        ...
        replace(
            ...
            required_lineage=tuple(
                edge for edge in INTEROP_ONTOLOGY_V1.required_lineage if edge != ("Graph", "Run")
            ),
        )
