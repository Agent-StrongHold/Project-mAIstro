...
async def test_ensure_schema_provisions_the_table() -> None:
    """A configured pool gets durable claims even before the alembic chain
    (038 base, 039 fencing) runs: wiring provisions the table itself, loudly
    better than the restart-unsafe in-memory fallback the previous wiring
    reached for."""
    pool = FakePool()
    ...
    # The provisioned shape mirrors migrations 038+039, column for column.
