...
@pytest.fixture(params=["America/New_York", "Pacific/Kiritimati"])
...
@pytest.mark.asyncio
async def test_sqlite_import_normalizes_a_naive_created_at_to_utc_not_the_local_zone(
    tmp_path, non_utc_local_timezone: str
) -> None:
    import aiosqlite
    ...
    naive_created_at = datetime(2024, 2, 3, 4, 5)
    conn = await aiosqlite.connect(tmp_path / "workspace-import-naive.db")
    try:
        ...
        assert reloaded.created_at == datetime(2024, 2, 3, 4, 5, tzinfo=UTC)
    finally:
        await conn.close()
...
@pytest.mark.asyncio
async def test_postgres_import_normalizes_a_naive_created_at_to_utc_not_the_local_zone(
