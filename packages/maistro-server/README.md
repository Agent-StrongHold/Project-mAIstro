...
- **Hive Conductor shell** — `src/maistro_server/static/hive/` at **`/conductor/`**
  (health + tasks against the same origin: `GET /health`, `GET /health/live`,
  `GET /health/startup`, `GET /health/ready`, `GET`/`POST /tasks`).
