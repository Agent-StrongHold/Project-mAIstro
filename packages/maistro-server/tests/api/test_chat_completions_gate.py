...
async def test_a_cancelled_admission_compensates_the_persisted_run_and_propagates(
    container: object, wired: InMemoryRunStore
) -> None:
    """A disconnect mid-admission compensates the persisted Run, then raises.

    This helper admits directly, before `route_request` can adopt the Run, so
    the seam's own compensation never sees it. `CancelledError` is not an
    `Exception`: without its own handler the CREATED/QUEUED Run outlives the
    request with no later seam to own it.
    """
    import asyncio
    ...
    async def _cancel_at_queued(run_id: str, target: RunStatus, **kwargs: object):
        if target is RunStatus.QUEUED:
            admitted_ids.append(run_id)
            raise asyncio.CancelledError()
        return await real_transition(run_id, target, **kwargs)  # type: ignore[arg-type]

    wired.transition_run = _cancel_at_queued  # type: ignore[method-assign]
    request = chat_api.ChatCompletionRequest(
        messages=[chat_api.ChatMessage(role="user", content="hi")]
    )

    with pytest.raises(asyncio.CancelledError):
        await chat_api._admit_turn(request, None)

    (run_id,) = admitted_ids
    compensated = await wired.get_run(run_id)
    assert compensated is not None
    assert compensated.status is RunStatus.CANCELLED
    assert compensated.error == ADMISSION_INCOMPLETE


async def test_a_cancellation_before_persistence_compensates_nothing(
    container: object, wired: InMemoryRunStore
) -> None:
    """No persisted Run means no compensation: the raise simply propagates."""
    import asyncio

    class _CancelledAdmission:
        async def admit(self, *_args: object, **_kwargs: object) -> None:
            raise asyncio.CancelledError()

    container.chat_admitter = _CancelledAdmission()  # type: ignore[attr-defined]
    request = chat_api.ChatCompletionRequest(
        messages=[chat_api.ChatMessage(role="user", content="hi")]
    )
    ...
    assert list(wired._runs.values()) == []  # type: ignore[attr-defined]
