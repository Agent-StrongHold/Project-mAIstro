...
def _classify_path(path: str, out: dict[str, bool]) -> None:
    ...
    if (core and any(token in path for token in ("event", "durable"))) or _under(path, "tests/events", "tests/durable_events"):
    ...
    if hive or server or _under(path, "docker-compose.yml", "docker-compose", "tests/e2e"):
        out["hive_e2e"] = True
    if _under(path, "packages") and (path.endswith("pyproject.toml") or "/src/" in path or path.endswith("/__init__.py")):
        out["wheel_imports"] = True
    if (
        path.startswith("Dockerfile")
        or path.startswith(".dockerignore")
        or path == "README.md"
        or _under(path, "packages", "alembic", "scripts", "tests", "formal", "tools", "quality", "agents", "templates", "docs", "sbx", ".github")
    ):
