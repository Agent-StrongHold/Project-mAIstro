...
def test_pull_request_resolves_exact_base_sha() -> None:
    assert resolve_base_revision("pull_request", {"pull_request": {"base": {"sha": SHA_A}}}) == SHA_A


def test_merge_group_resolves_exact_base_sha() -> None:
    assert resolve_base_revision("merge_group", {"merge_group": {"base_sha": SHA_B}}) == SHA_B
...
def test_pull_request_does_not_fall_back_to_merge_group_base() -> None:
    with pytest.raises(BaseRevisionError, match=r"pull_request\.base\.sha"):
        resolve_base_revision("pull_request", {"merge_group": {"base_sha": SHA_B}, "pull_request": {"base": {}}})
...
def test_environment_resolver_reads_the_github_event_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    event = tmp_path / "event.json"
...
def test_environment_resolver_requires_event_name(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_cli_main_prints_resolved_sha(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    event = tmp_path / "event.json"
...
def test_cli_main_reports_fail_closed_error(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    monkeypatch.delenv("GITHUB_EVENT_NAME", raising=False)
    monkeypatch.delenv("GITHUB_EVENT_PATH", raising=False)
    assert main() == 2
